...
function Disp() {
        ...
        switch (state) {
            ...
            case EN_STATE.PKHAND_JUDGE:
                if (isBef) { }
                    ...
                    Disp.showPKHandStr("", G_PLAYER_ID.PLAYER); //ポーカーハンドクリア
                    Disp.showPKHandStr("", G_PLAYER_ID.DEALER);
                    Disp.showPKHandResult("");                  //勝敗結果クリア
    ...
    //カードの画像を消す(１枚)
    //**********************************
    Disp.hiddenCardImg = function (arg_id) {
        var id = arg_id;
        var elm = document.getElementById(id);
        if (elm == null) alert(id + "は存在しません");
    ...
    //手札を画面に反映
    //**********************************
    Disp.applyHand = function (arg_hand, arg_playerid) {
        var playerid = arg_playerid || "";
        for (var i = 0; i < arg_hand.length; i++) {
            if (arg_hand[i].index == Card.NONE.IDX) Disp.hiddenCardImg("imgCard" + playerid + i);
            else Disp.changeCardImg("imgCard" + playerid + i, arg_hand[i].imgStr);
        }
    }
    ...
    //画面から手札クリア(非表示)
    //**********************************
    Disp.clearHand = function (arg_hand, arg_playerid) {
        var playerid = arg_playerid || "";
        for (var i = 0; i < arg_hand.length; i++)
            Disp.hiddenCardImg("imgCard" + playerid + i);
    }

    //**********************************
    //ポーカーハンドを画面に表示
    //**********************************
    Disp.showPKHandStr = function (arg_PKHand, arg_playerid) {
        var playerid = (arg_playerid != undefined) ? arg_playerid : "";
        var elm = document.getElementById("PKHandStr" + playerid);
    ...
    //勝敗結果を画面に表示
    //**********************************
    Disp.showPKHandResult = function (arg_cmpPKHand) {
        var elm = document.getElementById("PKHandStrResult");
        switch (arg_cmpPKHand) {
            case PokerHand.CMP.WIN:
                elm.innerHTML = "あなたの勝ち"; elm.style.color = "red";
                break;
            case PokerHand.CMP.DROW:
                elm.innerHTML = "引き分け"; elm.style.color = "black";
                break;
            case PokerHand.CMP.LOSE:
                elm.innerHTML = "あなたの負け"; elm.style.color = "blue";
                break;
            default:
                elm.innerHTML = "&nbsp;"; elm.style.color = "black";
                break;
        }
    }
    ...
    Disp.playSEByPKHand = function (arg_PKHand) {
        ...
        if (arg_PKHand.PKHand <= PokerHand.THREECARD) { seNum = G_EN_SE.WIN; }
    ...
    //ゲーム結果別にSEを出す
    //**********************************
    Disp.playSEByGame = function (arg_cmpPKHand, arg_PKHand) {
        switch (arg_cmpPKHand) {
            case PokerHand.CMP.WIN:
                Disp.playSEByPKHand(arg_PKHand); 
                break;
            case PokerHand.CMP.DROW: 
                break;
            case PokerHand.CMP.LOSE:
